/*
 * @Author: liziwei01
 * @Date: 2022-04-24 20:56:50
 * @LastEditors: liziwei01
 * @LastEditTime: 2022-04-24 20:56:56
...
func (ut *UTime) Str2Timestruct(str string, format ...string) (time.Time, error) {
	...
	if len(str) != len(f) {
		return time.Now(), fmt.Errorf("Str2Timestruct: parameter format error")
	}

	return time.ParseInLocation(f, str, Uptime.Location())
}
...
func (ut *UTime) Str2Timestamp(str string, format ...string) (int64, error) {
	tim, err := ut.Str2Timestruct(str, format...)
	if err != nil {
		return 0, err
	}
...
func (ut *UTime) EndOfMonth(date time.Time) time.Time {
	return ut.StartOfMonth(date).AddDate(0, 1, 0).Add(-time.Nanosecond)
}
...
func (ut *UTime) EndOfYear(date time.Time) time.Time {
	return ut.StartOfYear(date).AddDate(1, 0, 0).Add(-time.Nanosecond)
}
...
func (ut *UTime) EndOfWeek(date time.Time, weekStartDay ...time.Weekday) time.Time {
	return ut.StartOfWeek(date, weekStartDay...).AddDate(0, 0, 7).Add(-time.Nanosecond)
}
...
func (ut *UTime) IsDate2time(str string) (bool, int64) {
	...
	tim, err := Time.Str2Timestamp(str)
	if err != nil {
		return false, 0
	}
